"""Private LUMA image service.

The bundled provider is intentionally lightweight so the distributed system can
be tested without a GPU. Replace ``generate_development_image`` and
``edit_development_image`` with a real model adapter while keeping the HTTP
contract unchanged.
"""
